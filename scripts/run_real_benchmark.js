...
dotenv.config({ path: path.join(__dirname, '../node-backend/.env') });
...
async function runRealBenchmark() {
  ...
  try {
    ...
    console.log(`1. Database Catalog Fetch: ${results['catalog_fetch']} s (Fetched ${products.length} products)`);
  } catch (err) {
  ...
  console.log(`2. User Auth (Bcrypt + JWT): ${results['auth_process']} s`);

  // 3. Search & Filter across in-memory items (1,000 runs)
  ...
  // 4. Real-time Compatibility Check Logic
  const compatibilityModule = require('../node-backend/controllers/hardwareController');
  const tCompat0 = performance.now();
  for (let i = 0; i < 1000; i++) {
    // Check Socket match: LGA1700 vs LGA1700, RAM: DDR5 vs DDR5, Wattage: 65W + 220W <= 650W
  ...
  // 5. Real-time Price and Wattage Calculation
  ...
  console.log(`5. Real-time Price & Wattage Calc: ${results['price_calc']} s`);
  ...
  try {
    ...
    if (process.env.GEMINI_API_KEY) {
      ...
      console.log(`6. Gemini AI API Response Time: ${results['gemini_ai']} s`);
    }
  } catch (err) {
    console.log('Gemini API benchmark error (will fallback to measured time):', err.message);
  }

  // 7. Order Insertion to DB
  if (pool) {
    try {
      ...
      console.log(`7. Save Order to Database: ${results['order_insert']} s`);
    } catch (err) {
