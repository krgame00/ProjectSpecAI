...
// Relationships using Draw.io Entity Relation notation
// 1:N -> startArrow=ERmandOne; endArrow=ERoneToMany;
// 1:1 -> startArrow=ERmandOne; endArrow=ERmandOne;
const relationships = [
  { from: 'tbl_users', to: 'tbl_orders', type: '1:N', exitX: 1, exitY: 0.3, entryX: 0, entryY: 0.3 },
  { from: 'tbl_orders', to: 'tbl_order_items', type: '1:N', exitX: 1, exitY: 0.3, entryX: 0, entryY: 0.3 },
  { from: 'tbl_categories', to: 'tbl_products', type: '1:N', exitX: 1, exitY: 0.3, entryX: 0, entryY: 0.3 },
  { from: 'tbl_products', to: 'tbl_order_items', type: '1:N', exitX: 1, exitY: 0.15, entryX: 0, entryY: 0.8 },
  { from: 'tbl_products', to: 'tbl_spec_cpu', type: '1:1', exitX: 1, exitY: 0.25, entryX: 0, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_mobo', type: '1:1', exitX: 1, exitY: 0.45, entryX: 0, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_ram', type: '1:1', exitX: 1, exitY: 0.65, entryX: 0, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_gpu', type: '1:1', exitX: 1, exitY: 0.85, entryX: 0, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_storage', type: '1:1', exitX: 1, exitY: 0.95, entryX: 0, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_psu', type: '1:1', exitX: 0, exitY: 0.85, entryX: 1, entryY: 0.5 },
  { from: 'tbl_products', to: 'tbl_spec_case', type: '1:1', exitX: 0.5, exitY: 1, entryX: 0.5, entryY: 0 }
];

function generateERDrawioXML() {
  ...
  tables.forEach(tbl => {
    ...
    // Render Each Column Row (Draw.io ER Table Row with PK/FK separation)
  ...
  // Render ER Crow's Foot Connector Lines
  relationships.forEach((rel, idx) => {
    const isOneToMany = rel.type === '1:N';
    const startArrow = 'ERmandOne';
    const endArrow = isOneToMany ? 'ERoneToMany' : 'ERmandOne';
    const label = rel.type;

    const edgeStyle = `edgeStyle=orthogonalEdgeStyle;fontSize=11;fontStyle=1;fontColor=#000000;strokeColor=#000000;strokeWidth=1.5;startArrow=${startArrow};startSize=8;endArrow=${endArrow};endSize=8;labelBackgroundColor=#ffffff;labelBorderColor=#000000;exitX=${rel.exitX};exitY=${rel.exitY};entryX=${rel.entryX};entryY=${rel.entryY};`;
    
    cells += `        <mxCell id="rel_${idx + 1}" value="${escapeXml(label)}" style="${edgeStyle}" edge="1" parent="1" source="${rel.from}" target="${rel.to}">
...
const erXml = generateERDrawioXML();
const targetPath = 'C:/Users/PC/Downloads/smart_pc_builder_er_diagram.drawio';
fs.writeFileSync(targetPath, erXml, 'utf8');
console.log('Successfully wrote Native Draw.io ER Diagram (Crow\'s Foot) to:', targetPath);
