...
## 🔄 Phase 3: การรันสแครปเปอร์แบบเต็มรูปแบบ (Full Catalog Scraping) [แผนงานขั้นถัดไป 🎯]

### 3.1 การสแครปข้อมูลสินค้าครอบคลุมทุกหมวดหมู่ (Full Multi-Category Scraping)
- **หมวดหมู่ที่ต้องดึงเพิ่ม:**
  1. **CPU:** Intel (LGA1700, LGA1851), AMD (AM4, AM5)
  2. **Mainboard:** B650, B760, Z790, X670, B550 ฯลฯ
  3. **GPU:** NVIDIA RTX 40 Series, AMD Radeon RX 7000 Series
  4. **RAM:** DDR4 / DDR5 (Capacity, Bus Speed, Latency)
  5. **Storage:** M.2 NVMe SSD, SATA SSD, HDD
  6. **Power Supply (PSU):** Wattage, 80+ Efficiency Rating, Modular
  7. **Case & Cooling:** Form Factor, Fan Support, Radiator Support

### 3.2 การรัน Background Automated Scraper
- สร้างสคริปต์ `scripts/run_full_deep_scraper.py` สำหรับรันสแครปสินค้าทีละหมวดหมู่อย่างต่อเนื่อง
- มีระบบ Rate-Limiting และ Retry Mechanism เพื่อไม่ให้โดนบล็อก IP
- บันทึก Log การทำงานและรายงานจำนวนสินค้าที่ Insert/Update ใหม่ใน MySQL

---

## 🎨 Phase 4: การปรับปรุง UI/UX หน้าจัดสเปค & Admin Dashboard [แผนงานขั้นถัดไป 🎯]

- [ ] **4.1 แสดงผล Deep Specs ใน Modal รายละเอียดสินค้า (Product Detail Modal)**
  - ปรับปรุง `Frontend/src/views/Builder.vue` ให้แสดงผลตารางสเปคยิบย่อยจาก `specifications` (JSON / Typed Spec Tables) ให้สวยงาม สไตล์ Supabase
  - แสดงรูปภาพ HD 900px ใน Modal พร้อมปุ่ม Zoom/Gallery
...
| **Phase 3** | Full Multi-Category Scraping (CPU, GPU, MB, RAM, SSD, PSU, Case) | ⏳ พร้อมรัน | 10 ส.ค. 2026 |
| **Phase 4** | UI Spec Display & Dynamic Compatibility Checker | 🔜 ถัดไป | 11 ส.ค. 2026 |
