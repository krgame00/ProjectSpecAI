...
from patchright._impl._credentials import Credentials
from patchright._impl._debugger import Debugger
from patchright._impl._dialog import Dialog
from patchright._impl._disposable import Disposable, DisposableStub
                ...
                try:
                    if dialog.type == "beforeunload":
                        await self._connection.wrap_api_call(
                            lambda: dialog.accept(), is_internal=True
                        )
                    else:
                        await self._connection.wrap_api_call(
                            lambda: dialog.dismiss(), is_internal=True
                        )
                except Exception:
                    pass

            asyncio.create_task(handle_dialog())
