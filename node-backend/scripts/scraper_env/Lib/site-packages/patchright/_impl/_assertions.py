...
from patchright._impl._js_handle import parse_value
...
def _record_soft_or_raise(error: AssertionError, is_soft: bool) -> None:
    ...
    if is_soft:
        if _soft_errors is None:
            raise RuntimeError(
                "expect.soft(...) requires pytest-playwright>=0.7.3 (or pytest-playwright-asyncio>=0.7.3). Upgrade the plugin, or use a regular expect(...) assertion."
            )
