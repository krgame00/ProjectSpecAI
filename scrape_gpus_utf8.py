...
def scrape_gpu_data(url):
    ...
    for t in price_elems:
        if re.search(r'\d{1,3}(?:,\d{3})+\.\d{2}', t) or re.search(r'\d{4,}\.\d{2}', t):
            try:
                p = clean_price(t)
...
def main():
    ...
    for i, url in enumerate(urls):
        ...
        if data and data['price'] > 0:
            ...
            new_model = re.sub(r'^VGA\s*\(\s*การ์ดจอ\s*\)\s*', '', new_model, flags=re.IGNORECASE)
            brand_pattern = r"^" + re.escape(data['brand']) + r"\s+"
            new_model = re.sub(brand_pattern, '', new_model, flags=re.IGNORECASE).strip()
            data['model'] = new_model
            
            # Check for duplicates
            cursor.execute("SELECT id FROM products WHERE category_id = %s AND brand = %s AND model = %s", (DB_GPU_CAT, data['brand'], data['model']))
            if cursor.fetchone():
                print(f"Skipped (Duplicate): {data['brand']} {data['model']}")
                continue
