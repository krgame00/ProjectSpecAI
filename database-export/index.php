<?php
/**
 * Smart PC Builder — MySQL Real-time Database Inspector (PHP)
 * ดึงข้อมูลสดจาก MySQL อัตโนมัติทุกครั้งที่กด Refresh
 */
...
$db_name = getenv('DB_NAME') ?: 'smart_pc_builder';
...
try {
    ...
    die("<div style='color:red;padding:2rem;font-family:sans-serif;'>❌ ไม่สามารถเชื่อมต่อ MySQL ได้: " . htmlspecialchars($e->getMessage()) . "</div>");
}

// 2. ดึงตารางทั้งหมดในฐานข้อมูล
...
foreach ($tables as $tableName) {
    ...
    // Mask รหัสผ่าน และแปลงคอลัมน์ JSON
    ...
    $totalRecords += count($sanitizedRows);
}
...
<head>
  ...
  <title>Real-time DB Inspector — <?= htmlspecialchars($db_name) ?></title>
...
<body>
  <div class="header">
    <div class="title-area">
      ...
      <div>
        <div class="db-title"><?= htmlspecialchars($db_name) ?> <span class="live-badge">REALTIME PHP</span></div>
        <div class="db-sub">อัปเดตสดจาก MySQL: <?= $generatedAt ?></div>
      </div>
    </div>
    <div class="stats-area">
      <div>ตารางทั้งหมด: <span class="stat-badge"><?= count($databaseData) ?></span></div>
  ...
  <script>
    const dbData = <?= json_encode($databaseData, JSON_UNESCAPED_UNICODE) ?>;
