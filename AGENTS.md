...
- **Authentication**: JWT Auth. For protected API routes, always include the `Authorization: Bearer *** header (retrieve from `localStorage.getItem('token')`).
...
### Issue tracker
Issues live in GitHub Issues (krgame00/ProjectSpecAI) via the `gh` CLI. See `docs/agents/issue-tracker.md`.

### Triage labels
Canonical five triage labels as-is: needs-triage, needs-info, ready-for-agent, ready-for-human, wontfix. See `docs/agents/triage-labels.md`.
